...
class BackgammonCV:
    def __init__(self, enable_ui=False):         # ← add enable_ui param
        ...
        # Fix: absolute path so uvicorn can find the model
    ...
    def loadTemplate(self):
        # Fix: absolute path, search two candidate locations
        _base = Path(__file__).resolve().parent
        candidates = [
    ...
    def alignTemplate(self, image):
        # if self.enable_ui and self.board_scene:    # ← guard pygame
    ...
    def processStableBoard(self, stable_board):
        ...
        # Fix: absolute path for frame log files
        # _base = Path(__file__).resolve().parent
        # _out = _base.parent / "data" / "movements"
        # _out.mkdir(parents=True, exist_ok=True)
        # with open(str(_out / f"frame_{self.frame_index}.txt"), "w") as f:
        #     f.write(stable_board.exportJSON(as_string=True))
    ...
    def updateStableBoard(self):
        ...
        if self.candidate_count >= self.STABLE_FRAMES:
            # if self.enable_ui and self.board_scene:   # ← guard board_scene
            #     self.board_scene.updateBoard(self.board)
    ...
    def detect(self):
        while True:
            if not self.template_aligned:
                ...
                if not self.template_aligned:      # ← stop if alignment failed
            ...
            if self.enable_ui:                     # ← guard imshow
                cv2.imshow("Source", self.overlay)
